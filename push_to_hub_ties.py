...
def generate_model_string(output_dir):
    ...
    for model_entry in config.get('models', []):
        ...
        # Extract the weight
        density = model_entry.get('parameters', {}).get('density')
        densities.append(str(density))
    ...
    densities_part = "-".join(densities)
    result_string = f"{revs_part}_ties_densities-{densities_part}_lambda-{lambda_value}"
