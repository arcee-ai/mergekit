...
def prune_n_m_structural(
    tensor: torch.Tensor, n_val: int, m_val: int
    ...
    if padding > 0:
        flat_tensor_padded = torch.cat(
            (
                flat_tensor_orig_values,
                torch.zeros(padding, device=device, dtype=tensor.dtype),
            )
        )
    else:
...
class CABSMerge(MergeMethod, BaseModel, frozen=True):
    ...
    method_pretty_name_arg: Optional[str] = Field(
        "Conflict-Aware N:M Sparsification", alias="method_pretty_name", exclude=True
    )
    method_reference_url_arg: Optional[str] = Field(
        "https://arxiv.org/abs/2503.01874", alias="method_reference_url", exclude=True
    )
    ...
    def check_default_n_m_consistency(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if n is not None and m is None:
            raise ValueError(
                "If 'default_n_val' is provided, 'default_m_val' must also be provided."
            )
        if m is not None and n is None:
            raise ValueError(
                "If 'default_m_val' is provided, 'default_n_val' must also be provided."
            )
        if n is not None and m is not None:
            if not (
                ...
                raise ValueError(
                    f"Invalid default n/m values: n={n}, m={m}. Ensure 0 <= n <= m and m > 0."
                )
        return values
    ...
    def parameters(self) -> List[ConfigParameterDef]:
        ...
        return [
            ConfigParameterDef(
                name="default_n_val", type="int", required=False, default_value=None
            ),
            ConfigParameterDef(
                name="default_m_val", type="int", required=False, default_value=None
            ),
            ConfigParameterDef(
                name="pruning_order",
                type="list[str]",
    ...
    def tensor_parameters(self) -> List[ConfigParameterDef]:
        return [
            ...
            ConfigParameterDef(
                name="n_val", type="int", required=False, default_value=None
            ),
            ConfigParameterDef(
                name="m_val", type="int", required=False, default_value=None
            ),
        ]
    ...
    def make_task(
        ...
        parameters: ImmutableMap[
            str, Any
        ],  # This map IS from the nested 'parameters:' block in YAML
        ...
        if "default_n_val" in parameters:
            ...
            if isinstance(val, int):
                default_n_val_global = val
            elif val is not None:
                logging.warning(f"Expected int for default_n_val, got {type(val)}")

        default_m_val_global: Optional[int] = None
        if "default_m_val" in parameters:
            val = parameters["default_m_val"]
            if isinstance(val, int):
                default_m_val_global = val
            elif val is not None:
                logging.warning(f"Expected int for default_m_val, got {type(val)}")
        ...
        if "pruning_order" in parameters:
            ...
            if isinstance(val, list) and all(isinstance(s, str) for s in val):
                ...
                logging.warning(
                    f"Expected list of strings for pruning_order, got {type(val)}"
                )
        ...
        if default_n_val_global is not None and default_m_val_global is None:
            raise ValueError(
                "If 'default_n_val' is provided in global parameters, 'default_m_val' must also be provided."
            )
        if default_m_val_global is not None and default_n_val_global is None:
            raise ValueError(
                "If 'default_m_val' is provided in global parameters, 'default_n_val' must also be provided."
            )
        if (
            default_n_val_global is not None and default_m_val_global is not None
        ):  # Both are provided
            if not (
                default_n_val_global >= 0
                and default_m_val_global > 0
                and default_n_val_global <= default_m_val_global
            ):
                raise ValueError(
                    f"Invalid global default n/m values: n={default_n_val_global}, m={default_m_val_global}. "
...
class CABSTask(Task[torch.Tensor]):
    ...
    def _resolve_n_and_m_for_model(self, model_ref: ModelReference) -> Tuple[int, int]:
        ...
        if model_ref in self.per_model_tensor_params:
            ...
            logging.debug(
                f"  Per-model params for {model_identifier_str}: {dict(current_model_inner_params_map)}"
            )

            if "n_val" in current_model_inner_params_map:
                per_model_n_raw = current_model_inner_params_map["n_val"]
                logging.debug(
                    f"    Raw per-model n_val: {per_model_n_raw} (type: {type(per_model_n_raw)})"
                )
            if "m_val" in current_model_inner_params_map:
                per_model_m_raw = current_model_inner_params_map["m_val"]
                logging.debug(
                    f"    Raw per-model m_val: {per_model_m_raw} (type: {type(per_model_m_raw)})"
                )
        ...
        def try_convert_to_int(val: Any, name: str) -> Optional[int]:
            ...
            if val is not None:  # Log if it's some other unexpected type
                logging.warning(
                    f"    Unexpected type for per-model {name}: {type(val)}. Expected int or float representing int."
                )
            return None
        ...
        if n_candidate is not None and m_candidate is not None:
            if n_candidate >= 0 and m_candidate > 0 and n_candidate <= m_candidate:
                logging.debug(
                    f"    Using per-model n_val={n_candidate}, m_val={m_candidate} for {model_identifier_str}"
                )
                return n_candidate, m_candidate
            else:
                logging.warning(
                    f"    Invalid per-model n_val/m_val after conversion: n={n_candidate}, m={m_candidate} "
        ...
        # If only one was provided or conversion failed for one, it's an incomplete/invalid pair
            ...
            logging.warning(
                f"    Incomplete or invalid per-model n_val/m_val after conversion: n={n_candidate}, m={m_candidate} "
        ...
        if (
            self.global_default_n_val is not None
            and self.global_default_m_val is not None
        ):
            # Global defaults already validated by CABSMerge.make_task
            logging.debug(
                f"    Using global default_n_val={self.global_default_n_val}, "
        ...
        logging.warning(
            f"    No valid per-model or global default n/m values for model {model_identifier_str} "
    ...
    @override
    def execute(
        self, tensors_arg: Dict[ModelReference, torch.Tensor], **_kwargs
    ) -> torch.Tensor:
        base_model_identifier_str = str(self.base_model_ref.model)
        if self.base_model_ref not in tensors_arg:
            logging.error(
                f"Base model '{base_model_identifier_str}' not found for '{self.current_weight_info.name}'."
            )
            device_str = (
                self.current_weight_info.device_str()
                if hasattr(self.current_weight_info, "device_str")
                and callable(self.current_weight_info.device_str)
                else "cpu"
            )
            dtype_val = (
                self.current_weight_info.dtype
                if hasattr(self.current_weight_info, "dtype")
                else torch.float32
            )
        ...
        merged_tensor_accumulator = (
            tensors_arg[self.base_model_ref]
            .clone()
            .to(device=target_device, dtype=target_dtype)
        )
        ordered_model_refs_for_ca: List[ModelReference] = []
        model_ref_by_string_id: Dict[str, ModelReference] = {
            str(ref.model): ref for ref in tensors_arg.keys()
        }
        ...
        if current_pruning_order_strings:
            for id_str_in_order in current_pruning_order_strings:
                if id_str_in_order == base_model_identifier_str:
                    continue
                if id_str_in_order in model_ref_by_string_id:
                    ordered_model_refs_for_ca.append(
                        model_ref_by_string_id[id_str_in_order]
                    )
                else:
                    logging.warning(
                        f"Model ID '{id_str_in_order}' from order not found for '{self.current_weight_info.name}'."
                    )
        else:
            sorted_non_base_string_ids = sorted(
                [
                    str(ref.model)
                    for ref in tensors_arg.keys()
                    if str(ref.model) != base_model_identifier_str
                ]
            )
            for id_str in sorted_non_base_string_ids:
                if id_str in model_ref_by_string_id:
                    ordered_model_refs_for_ca.append(model_ref_by_string_id[id_str])

        if not ordered_model_refs_for_ca:
            logging.info(
                f"No non-base models for '{self.current_weight_info.name}'. Returning base."
            )
            return merged_tensor_accumulator
        cumulative_param_mask = torch.zeros_like(
            merged_tensor_accumulator, dtype=torch.bool, device=target_device
        )

        for model_ref_current in ordered_model_refs_for_ca:
            if model_ref_current not in tensors_arg:
                logging.warning(
                    f"Tensor for '{str(model_ref_current.model)}' unavailable for '{self.current_weight_info.name}'."
                )
                continue
            fine_tuned_tensor_val = tensors_arg[model_ref_current].to(
                device=target_device, dtype=target_dtype
            )
            base_tensor_for_diff = tensors_arg[self.base_model_ref].to(
                device=target_device, dtype=target_dtype
            )
            ...
            if model_ref_current in self.per_model_tensor_params:
                ...
                if "weight" in inner_params:
                    ...
                    if isinstance(val, (int, float)):
                        scaling_coefficient = float(val)
                    else:
                        logging.warning(
                            f"Expected float for per-model weight, got {type(val)}"
                        )

            n_val_current, m_val_current = self._resolve_n_and_m_for_model(
                model_ref_current
            )
            task_vector_val = fine_tuned_tensor_val - base_tensor_for_diff
            available_params_mask = ~cumulative_param_mask
            candidate_task_vector = task_vector_val * available_params_mask.to(
                task_vector_val.dtype
            )
            pruned_task_vector, newly_retained_mask = prune_n_m_structural(
                candidate_task_vector, n_val_current, m_val_current
            )
            merged_tensor_accumulator += scaling_coefficient * pruned_task_vector.to(
                merged_tensor_accumulator.dtype
            )
